    ...
    <div className="min-h-screen bg-background">
      <PageShell title="Analytics" rightAction={
              ...
              <div className="space-y-2 flex-1">
                {categoryData.slice(0, 5).map((cat) => (
                  <div key={cat.name} className="flex items-center justify-between text-xs">
                    ...
                    <span className="text-muted-foreground">${cat.value.toFixed(0)}</span>
                  </div>
                ))}
              </div>
            </div>
        ...
        <div className="space-y-3">
          {topCategory && (
            <InsightCard title="Highest Spending" description={`${topCategory.name}: $${topCategory.value.toFixed(2)}`} type="negative" />
          )}
          <InsightCard title="Total Spent" description={`$${totalSpent.toFixed(2)} total across all categories`} type="info" />
        </div>
      </PageShell>
      <BottomNav />
    </div>
